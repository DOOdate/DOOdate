...
INVALID = make_aware(datetime.datetime(1, 1, 1, 0, 0, 0))
...
def relative_date_to_absolute(date: str) -> datetime.datetime:
    ...
    if date.startswith('week'):
        try:
            weeknum = int(date[4:])
            return make_aware(START + datetime.timedelta(weeks=max(weeknum-1, 0))) # Week 1 likely means 0 weeks offset from start
        except ValueError:
            return INVALID
        ...
        try:
            ...
            return make_aware(START + datetime.timedelta(weeks=max(ceil(lecturenum/2)-1, 0)))
        except ValueError:
            return INVALID
    return INVALID
...
def str_to_datetime(date: str) -> datetime.datetime:
    ...
    if 'tba' in date or 'tbd' in date:
        return INVALID
    try:
        return make_aware(dateparser.parse(date, fuzzy=True))
    except Exception:
        return INVALID
